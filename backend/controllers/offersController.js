...
exports.getFlightsWithinRange = [
    async (req, res) => {
        const { departure, destination, start_date, end_date } = req.query;
        try {
            const flights = await offerService.queryFlightsWithinRange(departure, destination, start_date, end_date);
